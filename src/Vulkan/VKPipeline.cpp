...
template <typename T>
VKPipeline* VKPipeline::get(ccBlendFunc func)
{
    return nullptr;
}

template <>
VKPipeline* VKPipeline::get<VkSpriteVertex>(ccBlendFunc func)
{
    static std::map<std::pair<unsigned int, unsigned int>, VKPipeline*> pipelines = {};
    std::pair<unsigned int, unsigned int> fnc = { (unsigned int)func.src, (unsigned int)func.dst };

    if (!pipelines.contains(fnc))
        pipelines[fnc] = create<VkSpriteVertex>(func);

    return pipelines[fnc];
}

template <>
VKPipeline* VKPipeline::get<ccV3F_C4B_T2F>(ccBlendFunc func)
{
    static std::map<std::pair<unsigned int, unsigned int>, VKPipeline*> pipelines = {};
    std::pair<unsigned int, unsigned int> fnc = { (unsigned int)func.src, (unsigned int)func.dst };

    if (!pipelines.contains(fnc))
        pipelines[fnc] = create<ccV3F_C4B_T2F>(func);
...
// std::unordered_map<VKPipeline*> ccV3F_C4B_T2F__Pipelines = {};

template <>
VKPipeline* VKPipeline::create<ccV3F_C4B_T2F>(ccBlendFunc func)
{
...
template <typename T>
VKPipeline* VKPipeline::create(ccBlendFunc func)
{
    return nullptr;
}

template <>
VKPipeline* VKPipeline::create<VkSpriteVertex>(ccBlendFunc func)
{
    auto vertSpv = VKShader::create("vert.spv"_spr, "vert.vert"_spr);
    auto fragSpv = VKShader::create("frag.spv"_spr, "frag.frag"_spr);
